...
def morph_erode(img):
    ...
    erode_img = np.zeros((h, w), dtype=np.uint8)
    img = np.pad(img, pad_width=1)
    h = h - k + 1
    w = w - k + 1
    for i in range(h):
        for j in range(w):
            res = img[i: i + k, j:j + k] * se
            erode_img[i, j] = np.min(res)
    return erode_img
...
def morph_dilate(img):
    ...
    k = 3
    se = np.ones((k, k), dtype=np.uint8)
    dilate_img = np.zeros((h, w), dtype=np.uint8)
    img = np.pad(img, pad_width=1)
    h = h - k + 1
    w = w - k + 1
    for i in range(h):
...
def boundary(img):
    ...
    # TO DO: implement your solution here
    bound_img = np.subtract(denoise(img),morph_erode(denoise(img)))
    return bound_img
